...
class App {
  ...
  _loadMap(position) {
    ...
    // initialize the map centered at user's location with a zoom level
  ...
  _toggleElevationField() {
    //turns on or the elevation display
  ...
  _newWorkout(e) {
    ...
    if (type === 'running') {
      ...
      if (
        !validIputs(distance, duration, cadence) || !allPositive(distance, duration, cadence)
      )
    ...
    if (type === 'cycling') {
      ...
      // validate all input elements are finite, positive, working
      if (
        !validIputs(distance, duration, elevation) || !allPositive(distance, duration)
      )
        return alert ('Inputs have to be postive numbers!!');
        // create the running object with map coordinates and form data
  ...
  _getLocalStorage() {
    ...
    console.log('Workouts restores as proper objects', this.#workouts);
  }
}
const app = new App();
